...
export default{
  ...
  methods:{
    search(){
      axios.get("http://api.cat-shop.penkuoer.com/api/v1/admin/users/"+this.input,{
      ...
      //获取用户收获地址
      axios.get("http://api.cat-shop.penkuoer.com/api/v1/admin/addresses/"+this.input,{
        headers:{'authorization':'Bearer '+this.token}
      }).then((data)=>{
...
export default{
  ...
  methods:{
    search(){
      this.$axios.get("http://api.cat-shop.penkuoer.com/api/v1/admin/users/"+this.input,{
        headers:{'authorization':'Bearer '+this.token}
      }).then((data)=>{
        if(data.data._id!==undefined){
          this.data=data.data
      ...
      //获取用户收获地址
      this.$axios.get("http://api.cat-shop.penkuoer.com/api/v1/admin/addresses/"+this.input,{
        headers:{'authorization':'Bearer '+this.token}
      }).then((data)=>{
